        ...
        if (options.highlight || options.hydrateWithESResults) {
            hits.forEach(doc => {
                ...
                if (options.highlight) {
                    doc.set('_highlight', results.hits[idx].highlight, {
                        strict: false
                    });
                }
                if (options.hydrateWithESResults) {
                    // Add to doc ES raw result (with, e.g., _score value)
                    doc.set('_esResult', results.hits[idx], {
                        strict: false
                    });
                    if (!options.hydrateWithESResults.source && doc._esResult) {
                        // Remove heavy load
                        doc.set(
                            '_esResult._source',
                            {},
...
function Mongoosastic(schema, pluginOpts) {
   ...
   */
    ...
    schema.statics.synchronize = function synchronize(inQuery, inOpts) {
        ...
        let _saveOnSynchronize =
            inOpts && inOpts.saveOnSynchronize !== undefined
                ? inOpts.saveOnSynchronize
                : saveOnSynchronize;
    ...
    schema.statics.esSearch = function(inQuery, inOpts, inCb) {
        ...
        opts.hydrateOptions =
            opts.hydrateOptions || defaultHydrateOptions || {};
